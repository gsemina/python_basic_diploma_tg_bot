...
class Calendar:
    ...
    def create_calendar(
        ...
        """
        Создание и постройка клавиатуры с календарём.

        :param name:
        :param year: Год для использования в календаре, если вы не используете текущий год.
        :param month: Месяц для использования в календаре, если вы не используете текущий месяц.
        :return: Возвращаем календарь InlineKeyboardMarkup .
        """
    ...
    def new(self, *args, **kwargs) -> str:
        ...
        for part in self._part_names:
            ...
            if self.sep in value:
                raise ValueError(
                    f"Символ {self.sep!r} определяется как разделитель "
                    f"и не может использоваться в значениях частей"
                )
