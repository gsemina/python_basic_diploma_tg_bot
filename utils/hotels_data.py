...
def search_and_show_hotel(message: Message, data: Dict) -> None:
    """
    Функция делает запрос на поиск информации об отели и
    выдает в чат подробные данные (адрес, фотографии).
    ...
    response_hotels = api_request.request.request('POST', url, payload)
    ...
    if response_hotels.status_code == 200:
        ...
        hotels = data_json.search_hotel.search_hotels(response_hotels.text, data['command'],
                                                       data['start_distance_to_center'],
                                                      data['end_distance_to_center'])
        if 'error' in hotels:
        ...
        for hotel in hotels.values():
            ...
            if count < int(data['quantity_hotels']):
                ...
                get_summary = api_request.request.request('POST', summary_url, summary_payload)
                logger.info(f'Сервер вернул ответ {get_summary.status_code}')
                if get_summary.status_code == 200:
                    summary_info = data_json.search_hotel_detale.hotel_info(get_summary.text)
                    ...
                    medias = []
                    links_to_images = []
                    # выбираем рандомно фото, т к фото бывает больше, чем запросил пользователь
                    try:
                    ...
                    # Ссылки на фото мы передаем в функцию для сохранения в базе данных
                    ...
                    database.add_users_bd.add_response(data_to_db)
